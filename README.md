...
**空行を境に「キャプション」と「リンク情報（メディアごと）」を分ける。**
...
リンク行は **`メディア名 URL 見出し`** をスペース区切りで書く:
- **メディア名** = ファイル名の stem。`video`／`image`（画像1枚のとき）／`image-1`・`image-2`…（複数画像）。
- **URL** = そのメディアのリンク先。
- **見出し** = 任意。省略すると「作品リリースページ」。スペースを含む文字列でもOK（3トークン目以降すべてが見出し）。

ルール:
- **1メディア = 1URL**。リンク行を書いたメディアだけリンク化され、ホバーで見出し+URLのオーバーレイ（少しグレーアウト）が出る。
- **書かなかったメディアはリンクもグレーアウトもしない**（従来表示のまま）。
- **空行が無ければ全体がキャプション**（リンクなし）。＝1行だけの従来 `caption.txt` はそのまま動く。
- メディア名が実在ファイルと一致しないリンク行は無視され、ビルド時に警告が出る。

例（動画と2枚目画像だけリンク、1枚目はリンクなし）:
